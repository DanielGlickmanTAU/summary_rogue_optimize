...
def add_summary_and_rouge(examples, do_sample, top_p, top_k, num_beams):
...
def eval_metric(dataset_split, exp, do_sample, top_p, top_k, num_beams):
    ds = dataset_split.map(lambda x: add_summary_and_rouge(x, do_sample, top_p, top_k, num_beams), batched=True,
...
def do_experiment(model, tokenizer, cnn, train_examples, examples_for_training_epoch, learning_rate, temperature,
                  precentile,
                  do_sample, top_p, top_k, num_beams, gradient_accumulation_steps, strikes=3):
    ...
    test_summaries = get_random_examples(cnn_train, examples_for_training_epoch).map(
        lambda x: add_summary_and_rouge(x, do_sample, top_p, top_k, num_beams),
        batched=True,
        batch_size=batch_size)
...
do_experiment(model, tokenizer, cnn,
              train_examples=3_000,
              examples_for_training_epoch=3_000,
              learning_rate=1e-05,
              temperature=8,
              precentile=0.02,
              do_sample=True,
